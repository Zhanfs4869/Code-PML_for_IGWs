...
the minimum time step that allows stable iterations in the PML-MB method.
"""
...
def run_one_dt(timestep, record_ke=False, ):
    ...
    def dSc0_profile(z, Dz):
        ...
        S = 0.5 * (1 - np.tanh((z - 0.3) / Dz))
        return S*dSc_lower + (1-S)*dSc_upper

    def log_profile(z, Dz):
        log_upper = -k2*m2/(1 - k2 * (z - 1))
        log_lower = 0
        S = 0.5 * (1 - np.tanh((z - 0.3) / Dz))
        return S*log_lower + (1-S)*log_upper

    def rho0_profile(z, Dz):
        rho_upper = (1 - k2 * (z - 1))**m2
        rho_lower = (1 + 0.7*k2)**m2
        S = 0.5 * (1 - np.tanh((z - 0.3) / Dz))
        return S*rho_lower + (1-S)*rho_upper

    # Setup of PML-MB region
    ...
    rho0['g'] = rho0_profile(z, Dz)  # keep your original setting
