"""
This is the script used to compute the effectiveness of PML-MB for different values of A 
when Dz=0.025, corresponding to Fig. 7(b)
"""
...
# Setup of the background profile
def dSc0_profile(z, Dz):
    # Define the entropy profile
    dSc_upper = (1/gamma)*k2/(1-k2*(z-1))
    dSc_lower = (1/gamma)*k2/(1+k2*0.7)
    S = 0.5 * (1 - np.tanh((z - 0.3) / Dz)) # Smooth function
    return S*dSc_lower + (1-S)*dSc_upper    # matching

def log_profile(z, Dz):
    # Define the log density profile
    log_upper = -k2*m2/(1 - k2 * (z - 1))
    log_lower = 0
    S = 0.5 * (1 - np.tanh((z - 0.3) / Dz)) # Smooth function
    return S*log_lower + (1-S)*log_upper    # matching

def rho0_profile(z, Dz):
    # Define the density profile
    rho_upper = (1 - k2 * (z - 1))**m2
    rho_lower = (1 + 0.7*k2)**m2
    S = 0.5 * (1 - np.tanh((z - 0.3) / Dz)) #Smooth function
    return S*rho_lower + (1-S)*rho_upper    #matching

# Setup of the PML-MB region
